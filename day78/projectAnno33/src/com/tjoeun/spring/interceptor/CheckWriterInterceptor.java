...
implements HandlerInterceptor{
  ...
  (UserDTO loginUserDTO,  BoardService boardService) {
  ...
  (HttpServletRequest request, 
   ...
   Object handler) throws Exception {
  	...
  	int content_idx 
  	  = Integer.parseInt(request.getParameter("content_idx"));
  	
  	// Request 객체 속에 저장된
  	// content_idx(글번호)에 해당하는
  	// ContentDTO 가져오기
  	//  ㄴ content_wirter_idx 가 필요해서..
  	
  	ContentDTO tmpContentDTO 
  	  = boardService.getContentInfo(content_idx);
  	
  	// 로그인한 사람과 글 쓴 사람의 회원번호(idx)가
  	// 일치하지 않으면 BoardController의
  	// "/board/not_writer" 로 이동하게 함
  	if(tmpContentDTO.getContent_writer_idx() 
  	   != loginUserDTO.getUser_idx() ) {
